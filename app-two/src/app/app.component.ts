...
export class AppComponent implements OnInit, OnDestroy {
  ...
  private async connectToSignalRHub() {
    ...
    this.hubConnection.on('2:NewChat', (messageId: string) => {
      this.getMessages();
    });
  ...
  private sendMessageToApi(message: string) {
    this.httpClient
      .post<Message>(this.backendUrl, { senderId: 2, body: message })
      .subscribe({
        next: (newMessage) => {
  ...
  private NotifySignalRForNewMessage(messageId: string) {
    this.hubConnection.send('NotifyNewMessage', 1, messageId);
  }
}
